...
impl ProtoRpc {
    ...
    pub fn get_input_param(&'_ self) -> Option<ParamType<'_>> {
        ParamType::parse(&self.input_param)
    }

    pub fn get_output_param(&'_ self) -> Option<ParamType<'_>> {
...
impl ProtoServiceDescription {
    ...
    pub fn read_proto_file(file_name: &str) -> Self {
        ...
        if let Err(err) = file {
            panic!("Can not open file: {file_name}. Error: {err:?}");
        }
            ...
            for token in ProtoTokensReader::new(line.as_str()) {
                match current_token {
                    ...
                    CurrentToken::RpcExpectingOutputParameter => {
                        ...
                        if !out_param_name.is_empty() {
                            out_param_name.push(' ');
                        }
                        out_param_name.push_str(token);
                    }
                    CurrentToken::Service => {
                        service_name = Some(format!("{token}Client"));
                        current_token = CurrentToken::None;
                    }
                }
            }
