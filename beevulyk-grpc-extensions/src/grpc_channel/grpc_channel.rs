...
use crate::{
    ...
    RequestBuilderWithInputStream, RequestBuilderWithStream,
};
...
impl<TService: Send + Sync + 'static> GrpcChannel<TService> {
    ...
    pub async fn get_service(&self) -> Result<TService, GrpcReadError> {
        let channel = self.get_channel().await?;
        let result = self.service_factory.create_service(channel);
    ...
    pub fn start_request_with_stream<
        TInputContract: Clone + Send + Sync + 'static,
    >(
        self,
        input_stream: impl futures_core::Stream<Item = TInputContract> + Send + Sync + 'static,
    ) -> RequestBuilderWithStream<TService, TInputContract> {
        let boxed_stream = Box::pin(input_stream);
        RequestBuilderWithStream::new(boxed_stream, self)
    }
    ...
    pub async fn execute_with_input_stream<
        TRequest: Send + Sync + 'static,
        TResponse: Send + Sync + 'static,
    ...
    >(
        ...
        let future = grpc_executor.execute(service, input_stream);
    ...
    pub async fn execute_with_input_stream_response_as_stream<
        TRequest: Send + Sync + 'static,
        TResponse: Send + Sync + 'static,
...
#[async_trait::async_trait]
pub trait RequestWithInputStreamGrpcExecutor<
    TService: Send + Sync + 'static,
    TRequest: Send + Sync + 'static,
    TResponse: Send + Sync + 'static,
>
...
#[async_trait::async_trait]
pub trait RequestWithInputStreamWithResponseAsStreamGrpcExecutor<
    TService: Send + Sync + 'static,
    TRequest: Send + Sync + 'static,
    TResponse: Send + Sync + 'static,
>
