...
pub fn generate_grpc_methods(
    ...
    for rpc in &proto_file.rpc {
        ...
        let get_channel = quote::quote!(self.channel.get_channel());
        ...
        let item = quote::quote! {
            ...
            ) -> Result<#output_data_type, beevulyk_grpc_extensions::GrpcReadError> {
...
fn get_func_in_data_type(data_type: Option<&super::ParamType<'_>>) -> proc_macro2::TokenStream {
    match data_type {
        Some(input_param) => {
            ...
            // True Stream support is available via start_request_with_stream() API
            input_param.get_input_param_type_token_vec()
        },
        None => {
            quote::quote! {()}
        }
