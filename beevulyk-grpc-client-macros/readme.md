client generator depends on beevulyk-grpc-extensions

```toml
beevulyk-grpc-extensions = { tag = "x.x.x", git = "https://github.com/BeeVulyk/beevulyk-grpc-extensions.git" }
```
...
use beevulyk_grpc_client_macros::generate_grpc_client;
...
* max_message_size _(optional)_: maximum request/response payload size in bytes. Sets both encoding and decoding limits. Defaults to `4 * 1024 * 1024`.
* max_decoding_message_size / max_encoding_message_size _(optional)_: override limits individually (bytes). Omitted sides fall back to `max_message_size`, then to the default.
...
impl beevulyk_grpc_extensions::GrpcClientSettings for SettingsReader {
