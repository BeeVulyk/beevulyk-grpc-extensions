...
```rust
    ...
    async fn get(
        ...
        return beevulyk_grpc_extensions::grpc_server::send_vec_to_stream(result, |dto| dto).await;
    }
```
...
```rust
let my_telemetry = beevulyk_grpc_extensions::get_telemetry(
            &request.metadata(),
            request.remote_addr(),
            #fn_name,
        );
