    ...
    for rpc in &proto_file.rpc {
        if let Some(input_param_type) = &rpc.get_input_param() {
            if let Some(output_param_type) = &rpc.get_output_param() {
                ...
                let input_param_invoke = input_param_type.get_input_param_invoke_token_vec();
        ...
        } else if let Some(output_param_type) = &rpc.get_output_param() {
...
fn get_interface_name(
    ...
    if input_param.is_stream() {
        if output_param.is_stream() {
            quote::quote!(
                beevulyk_grpc_extensions::RequestWithInputAsStreamWithResponseAsStreamGrpcExecutor
            )
        } else {
            quote::quote!(beevulyk_grpc_extensions::RequestWithInputAsStreamGrpcExecutor)
        }
    } else if output_param.is_stream() {
        quote::quote!(beevulyk_grpc_extensions::RequestWithResponseAsStreamGrpcExecutor)
    } else {
        quote::quote!(beevulyk_grpc_extensions::RequestResponseGrpcExecutor)
    }
}
...
fn get_name_fn_param_type_token(src: &ParamType<'_>) -> proc_macro2::TokenStream {
    // Use Vec-based type for backward compatibility
    // True Stream support is available via RequestWithInputStreamGrpcExecutor traits
    src.get_input_param_type_token_vec()
}
